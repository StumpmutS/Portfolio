/*
 Adjusted from https://github.com/malik-tillman/Fluid-JS
 */
...
import {initWebGL, activator, setDitherURL} from './initialization/initializer';
import {Behaviors} from "./types/behaviors";
import {ShaderPrograms} from "./initialization/gl-program";
import Pointer from "./initialization/pointer";
...
export default class Fluid {
     ...
     */
    activate() {
        activator(this.canvas, this.webGL, this.colorFormat, this.programs, this.pointers);
    }
     ...
     * @param canvas
     */
    applyBackground(type: ("gradient" | "image" | "solid"), value: { r: number, g: number, b: number }, options: {
        gradient: ("radial" | "conic" | "repeating-linear" | "repeating-radial" | "linear"),
        repeat: boolean,
        position: string,
        size: string,
        color: string,
