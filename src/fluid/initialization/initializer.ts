...
import {GLProgram, ShaderPrograms} from "./gl-program";
import {ColorFormats} from "../types/color-formats";
import {DoubleFrameBufferObject, FrameBufferObject} from "../types/frame-buffer-object";
...
export function initWebGL(canvas: HTMLCanvasElement) {
    ...
    pointers.push(new Pointer(defaults.behavior.fluid_color));

    /* Get webGL context */
    let webGL: WebGL2RenderingContext = canvas.getContext('webgl2', defaults.DRAWING_PARAMS) as WebGL2RenderingContext;
    const isWebGL2 = !!webGL;
    if (!isWebGL2) {
        webGL = (canvas.getContext('webgl', defaults.DRAWING_PARAMS) || canvas.getContext('experimental-webgl', defaults.DRAWING_PARAMS)) as WebGL2RenderingContext; // TODO: Refactor to force WebGL (not 2)
    }
    /* Get color formats */
    ...
    const SHADER = {
        ...
        displayInverted: compileShader(webGL.FRAGMENT_SHADER, defaults.SHADER_SOURCE.displayInverted),
     ...
     */
    function getFormats(): ColorFormats {
        /* Color Formats */
        let formatRGBA;
        let formatRG;
        let formatR;
        ...
        const HALF_FLOAT_TEXTURE_TYPE: GLenum = isWebGL2 ? webGL.HALF_FLOAT : halfFloat!.HALF_FLOAT_OES;
         ...
         * @param type
         */
        function getSupportedFormat(internalFormat: GLint, format: GLenum, type: GLenum): ({ internalFormat: GLint, format: GLenum } | null) {
     ...
     * @returns Programs used to render shaders
     */
    function formShaderPrograms(supportLinearFiltering: OES_texture_float_linear | null): ShaderPrograms {
        return {
            ...
            displayInvertedProgram: new GLProgram(SHADER.baseVertex, SHADER.displayInverted, webGL),
        };
    }
...
export function activator(canvas: HTMLCanvasElement, webGL: WebGL2RenderingContext, colorFormat: ColorFormats, PROGRAMS: ShaderPrograms, pointers: Pointer[]) {
    /* TODO: Retrieve haul style */
    const PARAMS = defaults.behavior;

    if (active) {
        let nPointers = [];
        nPointers.push(new Pointer(PARAMS.fluid_color));
        pointers = nPointers;
    }
    ...
    let density: DoubleFrameBufferObject;
     ...
     *
     */
    let ditheringTexture = PARAMS.embedded_dither ? createTextureAsync("/dither.png") : createTextureAsync(ditherURL);
    ...
    function init() {
        /* Color Formats */
        const texType: GLenum = colorFormat.halfFloatTexType;
        const rgba = colorFormat.formatRGBA;
        const rg = colorFormat.formatRG;
        const r = colorFormat.formatR;
        const filtering: GLint = colorFormat.supportLinearFiltering ? webGL.LINEAR : webGL.NEAREST;

        /* Set simulation and pointer width and height */
        let simRes = getResolution(PARAMS.sim_resolution ?? 0);
        let dyeRes = getResolution((PARAMS.dye_resolution ?? 0));
        let bloomRes = getResolution((PARAMS.bloom_resolution ?? 0));
     ...
     */
    function createDoubleFBO(w: number, h: number, internalFormat: GLint, format: GLenum, type: GLenum, param: GLint): DoubleFrameBufferObject {
    ...
    function createFBO(w: GLsizei, h: GLsizei, internalFormat: GLint, format: GLenum, type: GLenum, param: GLint): FrameBufferObject {
        ...
        if (!texture) {
            throw "Error creating WebGL texture"
        }
        if (!fbo) {
            throw "Error creating WebGL FrameBuffer"
        }
    ...
    function resizeDoubleFBO(target: DoubleFrameBufferObject, w: number, h: number, internalFormat: GLint, format: GLenum, type: GLenum, param: GLint) {
        target.read = resizeFBO(target.read, w, h, internalFormat, format, type, param);
        target.write = createFBO(w, h, internalFormat, format, type, param);
        return target;
    }

    function resizeFBO(target: FrameBufferObject, w: number, h: number, internalFormat: GLint, format: GLenum, type: GLenum, param: GLint) {
    ...
    function step(dt: GLfloat) {
        ...
        webGL.uniform1f(PROGRAMS.vorticityProgram.uniforms.curl, (PARAMS.curl ?? 0));
        ...
        webGL.uniform1f(PROGRAMS.clearProgram.uniforms.value, (PARAMS.pressure ?? 0));
        ...
        for (let i = 0; i < (PARAMS.pressure_iteration ?? 0); i++) {
        ...
        webGL.uniform1f(PROGRAMS.advectionProgram.uniforms.dissipation, (PARAMS.velocity ?? 0));
        blit(velocity.write.fbo);
        velocity.swap();
        ...
        webGL.uniform1f(PROGRAMS.advectionProgram.uniforms.dissipation, (PARAMS.dissipation ?? 0));
    ...
    function render(target: WebGLFramebuffer | null) {
        ...
        if (PARAMS.render_shaders) {
            ...
            if (PARAMS.invert_color) program = PROGRAMS.displayInvertedProgram;
    ...
    function applyBloom(source: FrameBufferObject, destination: FrameBufferObject) {
        ...
        let knee = (PARAMS?.threshold ?? 0) * (PARAMS?.soft_knee ?? 0) + 0.0001;
        let curve0 = (PARAMS.threshold ?? 0) - knee;
        let curve1 = knee * 2;
        let curve2 = 0.25 / knee;
        webGL.uniform3f(PROGRAMS.bloomPreFilterProgram.uniforms.curve, curve0, curve1, curve2);
        webGL.uniform1f(PROGRAMS.bloomPreFilterProgram.uniforms.threshold, (PARAMS.threshold ?? 0));
        ...
        webGL.uniform1f(PROGRAMS.bloomFinalProgram.uniforms.intensity, (PARAMS.intensity ?? 0));
        webGL.viewport(0, 0, destination.width, destination.height);
        blit(destination.fbo);
    }

    function splat(x: number, y: number, dx: GLfloat, dy: GLfloat, color: Rgb) {
        webGL.viewport(0, 0, simWidth, simHeight);
        ...
        webGL.uniform1f(PROGRAMS.splatProgram.uniforms.radius, (PARAMS.emitter_size ?? 0) / 100.0);
        blit(velocity.write.fbo);
        velocity.swap();
